...
size_t wav_read_int24(pcmfile_t *sndf, int32_t *buf, size_t num, int *map)
{
  ...
  bufi = (unsigned char *) (char *)buf + sizeof(*buf) * num - sndf->samplebytes * (num - 1) - sizeof(*buf);
...
size_t wav_read_int24_buf(pcmfile_t *sndf, int32_t *outbuf, size_t num, unsigned char *inputbuf)
{
    size_t i=0,j=0;
    int16_t bufi;
